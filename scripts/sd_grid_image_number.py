...
def adapt_font_size(width, height):
    diagonal = math.sqrt(width**2 + height**2)
    return int(diagonal / 8)
...
def on_ui_settings():
    ...
    opts.add_option("sd_grid_font_adaptive", OptionInfo(True, "Use adaptive font size for image number in grid", section=section))
    opts.add_option("sd_grid_font_size", OptionInfo(32, "Font size of the image number in grid (32px by default, adaptive by choice)", gr.Number, {"precision": 0}, section=section))
        ...
        for img in params.imgs:
            ...
            if hasattr(img, "already_saved_as"):
                ...
                if img_num_text.isdigit():
                    ...
                    img_num_font = ImageFont.truetype(Roboto if opts.sd_grid_font == "Default" else opts.sd_grid_font, adapt_font_size(width, height) if opts.sd_grid_font_adaptive else opts.sd_grid_font_size)
                    img_num_distance = 8
